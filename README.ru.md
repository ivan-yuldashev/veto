...
CASL — признанный лидер и хорошая библиотека, но она появилась до React Server Components и построена вокруг классов.

| | CASL | @vetojs |
|---|---|---|
| **Серверные компоненты** | `subject(obj)` **меняет** ваши объекты, чтобы пометить тип, — а такое RSC не сериализует | правила — обычный JSON, имя ресурса просто аргумент: ничего не оборачивается и не меняется |
| **Типы** | действия не привязаны к ресурсу, а формы приходится описывать вручную — `MongoAbility<[Actions, Subject]>` | действия, ресурсы и формы выводятся из одного объявления |
| **Пометка объекта** | `can("update", subject("Post", post))` | `can("update", "post", post)` |
| **Запросы к базе** | `accessibleBy` (Mongo / Prisma) | `ability.where()` → SQL через `@vetojs/drizzle`, с проверенной гарантией: запрос вернёт ровно то, что разрешает `can()` |
| **Зависимости** | несколько | ни одной |
| **Плохие данные** | условие может тихо посчитаться не в ту сторону | значение, не подходящее условию, даёт «неизвестно»: `allow` не даёт ничего, а `deny` всё равно срабатывает |
