...
| [`@vetojs/next`](packages/next) | ✅ Готов | [`createGuard`](docs/next.ru.md) — одна обёртка определяет пользователя, загружает строку, проверяет её и валидирует данные до того, как отработает ваш server action. |
