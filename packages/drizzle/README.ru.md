...
**Запрос вернёт ровно те строки, которые разрешает `can()`.** Не надмножество, которое придётся перепроверять в JavaScript, и не подмножество, тихо прячущее законные строки. Связи превращаются в подзапросы `EXISTS`, поэтому `author.role` или `comments.some.spam` работают в SQL так же, как в памяти.
