...
>
> The guard in this package never imported `next` or `react`, so it moved into the engine as **`@vetojs/core/guard`** — where it also serves HTTP handlers and agent tool calls. This package is now a re-export and will receive no further changes.
