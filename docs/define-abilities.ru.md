...
| Поле | Что означает |
|---|---|
| `schema` | собственные поля строки. `shape<T>()` — метка, которая ничего не делает во время работы и только несёт тип; передайте настоящую [Standard Schema](https://standardschema.dev), если хотите ещё и проверять данные — [см. ниже](#подставить-настоящую-схему-вместо-typet) |
