...
Вердикты те же, а размер перестаёт зависеть от числа тенантов: на политике общего примера при 50 воркспейсах **454 правила и 83 kB превращаются в 13 правил и 4.7 kB**, а `can()` — с 27.7 мкс в 1.0 мкс. Группируйте по тому, от чего правила действительно ветвятся (обычно это роль), а идентификаторы отдавайте оператору `in`.
