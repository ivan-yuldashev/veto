...
`load` имеет право вернуть пустоту: `findFirst` ничего не нашёл или id оказался чужим. Это отказ, и в журнал он приходит как `reason: "no row"`. Поэтому `ctx.row` типизирован строкой, а не «может быть строкой»: раз до обработчика дошло, строка нашлась.
...
| `row` | то, что вернул `load`, — строка, а не «может быть строка» |
...
Два отказа до правил не доходят, и о каждом сообщается там, где он случился.
Если `load` вернул не строку, в решении приезжает `reason: "no row"`: id был чужим
или ничьим, а в журнале это читается иначе, чем «политика отказала». Когда
никто не вошёл, пользователя нет — значит нет ни политики, ни решения; вместо
этого `onUnauthenticated` получает то, что пытались сделать, и это единственное
место, где такую попытку можно записать:
