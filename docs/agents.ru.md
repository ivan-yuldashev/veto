...
**Инструменту-эффекту всегда давайте `load`.** Без строки проверка по контракту оптимистична — истина, когда какой-то `allow` мог бы покрыть действие для *какой-то* строки, — а это неверный ответ перед необратимым шагом. Собрать строку стоит нескольких строк кода, и тогда гвард откажет ещё до того, как ваш обработчик появится.
